    ...
    this.activedRoute.queryParams.subscribe((resp: any) => {
      ...
      if (this.code_user) {
        ...
        this.authService.verifiedAuth(data).subscribe((r: any) => {
          if (r.message == 403) this.toastr.error('Validación', 'El código no pertenece a ningún usuario');
          if (r.message == 200) {
            this.toastr.success('Éxito', 'El correo ha sido verificado. Ya puedes entrar en la tienda');
