...
export class CreateAnidadoVariationsComponent {
  ...
  saveVariation(){
    ...
    if(this.precio_add < 0){
      this.toastr.error('Validación', 'El valor del precio debe ser mayor o igual a 0');
      return;
    }
    if(this.stock_add <= 0){
  ...
  this.attributeService.createVariationsAnidadas(data).subscribe((resp:any) => {
    ...
    if(resp.message == 403){
      ...
      this.toastr.success('Éxito', "se registró la variación ANIDADA correctamente");
  ...
  editVariation(variation:any){
    const modal = this.modalService.open(EditAnidadoVariationsComponent, { size: 'md', centered: true });
